...
impl Instruction {
    ...
    fn process(&self, grid: &mut [[bool; GRID_SIDE_LEN]; GRID_SIDE_LEN]) {
        {
            match self.kind {
                // Turn on anything that's off in range
                InstructionKind::On => {
                ...
                // Turn off anything that's on in range
                InstructionKind::Off => {
                    for row in &mut grid[self.from.y..self.to.y + 1] {
                        for cell in &mut row[self.from.x..self.to.x + 1] {
                            *cell = false;
...
fn main() {
    ...
    // GRID_SIDE_LEN x GRID_SIDE_LEN 2D array of "lights". true means the light is on
    let mut grid = [[false; GRID_SIDE_LEN]; GRID_SIDE_LEN];

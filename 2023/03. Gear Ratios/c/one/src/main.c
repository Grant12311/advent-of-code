...
/**
 * @brief Adds the int pointed to by value to the int pointed to by sum
 * 
 * @param key 
 * @param value 
 * @param sum 
 */
static void sumEntries(void* const key, void* const value, void* const sum)
{
    (void)key;
    *((int*)sum) += *((int*)value);
}
...
int main(const int argc, const char** const argv)
{
    ...
    for (size_t y = 0; y < gridHeight; ++y)
    {
        for (size_t x = 0; x < gridWidth; ++x)
        {
            ...
            // Process all neighbors of c if it is a symbol
            if (c != '.' && ispunct(c))
            {
    ...
    // Sum all part numbers (puzzle result)
    int sum = 0;
    bstMapForEach(&partNumbers, sumEntries, &sum);
    printf("Result: %d\n", sum);
